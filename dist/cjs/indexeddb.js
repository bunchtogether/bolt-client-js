...
const databasePromise = new Promise((resolve, reject) => {
  ...
  request.onerror = event => {
    _index.default.logger.error('Unable to open stored server database');
  ...
  request.onsuccess = function (event) {
    _index.default.logger.info('Opened stored server database');
...
async function getReadWriteObjectStore() {
  ...
  transaction.onabort = event => {
    _index.default.logger.error('Read-write stored server transaction was aborted');

    console.error(event); // eslint-disable-line no-console
  };

  transaction.onerror = event => {
    _index.default.logger.error('Error in read-write stored server transaction');
...
async function getStoredServersCallback() {
  ...
  return new Promise((resolve, reject) => {
    ...
    request.onerror = function (event) {
      _index.default.logger.error('Unable to get stored servers from indexedDB');
...
async function saveStoredServersCallback(servers) {
  ...
  for (let i = 0; i < servers.length; i += 1) {
    ...
    await new Promise((resolve, reject) => {
      ...
      request.onerror = function (event) {
        _index.default.logger.error('Unable to add stored servers to indexedDB');
...
async function clearStoredServersCallback() {
  ...
  await new Promise((resolve, reject) => {
    ...
    request.onerror = function (event) {
      _index.default.logger.error('Unable to clear stored servers from indexedDB');
