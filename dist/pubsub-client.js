...
export default class BoltPubSubClient extends EventEmitter {
  constructor(element                  , streamUrl       , baseUrl       , ipfs       ) {
    super();
  ...
  async openPubSub(streamUrl       ) {
    ...
    let messageDataQueue = [];
    let seqno = Buffer.from([0]);
  ...
  async setupMediaSource(element                  , mimetype        ) {
